...
if verbose:
    print("Building project={0} for board={1} using platform={2} and toolchain={3}.".format(
        project, name, platform, toolchain))
...
if (platform == 'cc2538'):
    ...
    env['lib_path'] += [os.path.join('#',
                                     board['lib_path'], board['platform'])]
...
class OpenMoteCC2538_bootloadThread(threading.Thread):
    ...
    def run(self):
        ...
        command = 'python3 ' + os.path.join(self.bsl_path, self.bsl_name) + \
            self.bsl_params + '{0} {1}'.format(self.port, self.binary)
        if (verbose):
...
def OpenMoteCC2538_expand(ports):
    ...
    if (len(ports) == 1):
        ...
        # /dev/ttyUSB[1-2] means bootload a range of ttyUSB ports
            ...
            first, last = sorted(
                map(int, ((port.split('['))[1].split(']')[0]).split('-')))
...
def BootloadFunc():
    if ((env['board'] == 'openmote-b')):
        ...
        raise SystemError(
            'Bootloading on board={0} unsupported.'.format(env['board']))
...
for folder in src_folders:
    ...
    output = env.SConscript(script, variant_dir=build,
